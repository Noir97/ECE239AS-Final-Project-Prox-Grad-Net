...
for epoch in range(epoch_num):
    ...
    if epoch % 5 == 0:
        vutils.save_image(torch.cat((torch.irfft(input_data, 2, onesided=False).detach(), torch.irfft(input_data_v, 2, onesided=False).detach(), output.detach(), output_v.detach(), label, label_v), 0), 'CS_MRI_results/epoch_{}.png'.format(epoch+1), nrow=8)